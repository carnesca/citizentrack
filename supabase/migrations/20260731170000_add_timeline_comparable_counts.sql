    ...
    coalesce((
      ...
      from (
        select
          ...
          count(a.*) filter (where a.submitted_on is not null and a.aktenzeichen_on is not null)::int as submission_to_az_comparable_cases,
          count(a.*) filter (where a.aktenzeichen_on is not null and a.certificate_received_on is not null and a.status = 'certificate_received')::int as az_to_certificate_comparable_cases,
          count(a.*) filter (where a.submitted_on is not null and a.certificate_received_on is not null and a.status = 'certificate_received')::int as submission_to_certificate_comparable_cases,
